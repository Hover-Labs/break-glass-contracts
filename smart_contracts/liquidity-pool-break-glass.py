...
# A break glass contract that can revert control of a governor in the Liquidity Pool.
class LiquidityPoolBreakGlass(sp.Contract):
  def __init__(
    self,
  ...
  def breakGlass(self, newGovernor):
    ...
    targetContractHandle = sp.contract(sp.TAddress, self.data.targetAddress, 'updateGovernorAddress').open_some()
...
def test():
  ...
  scenario += box.updateGovernorAddress(breakGlass.address).run(
...
sp.add_compilation_target("liquidity-pool-break-glass", LiquidityPoolBreakGlass())
