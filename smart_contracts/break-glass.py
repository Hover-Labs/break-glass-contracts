...
# A break glass contract that can revert control of a governor in Kolibri.
class BreakGlass(sp.Contract):
  def __init__(
    self,
  ...
  def breakGlass(self, newGovernor):
    ...
    targetContractHandle = sp.contract(sp.TAddress, self.data.targetAddress, 'setGovernorContract').open_some()
...
def test():
  ...
  scenario += box.setGovernorContract(breakGlass.address).run(
